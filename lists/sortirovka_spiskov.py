...
# for i in range(n - 1):
#
#     if swap == False:  # если по окончании внешнего цикла сигнальная метка приняла значение False, т.е. ни одного
#     обмена не было произведено, тогда программа прерывается
#         break
#
#     for j in range(n - i - 1):
#         if a[j] > a[j + 1]:
#             a[j], a[j + 1] = a[j + 1], a[j]
#
#             swap = True  # если в данном внутреннем цикле была произведена хотя бы одна перестановка во внутреннем
#             цикле, сигнальная метка принимает значение True
#         else:
